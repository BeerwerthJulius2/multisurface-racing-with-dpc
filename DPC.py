...
def main():  # after launching this you can run visualization.py to see the results
    ...
    model_to_use = 'dpc'  # options: ext_kinematic, pure_pursuit, dynamic, dpc
    ...
    planner_dpc = DPCAdapter(waypoints=waypoints)
    ...
    while not done:
        ...
        elif model_to_use == "dpc":
            if len(initial_inputs) < initialization_horizon or len(initial_outputs) < initialization_horizon:
                # Regulator step pure pursuit
                speed, steer_angle = planner_pp.plan(obs['poses_x'][0], obs['poses_y'][0], obs['poses_theta'][0],
                                                     work['tlad'],
                                                     work['vgain'])
